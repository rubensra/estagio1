...
local function ataqueChefe(chefe)
    som.temaChefe();
    local movimento2 = transition.to(chefe, { x = 260, time = 3000, transition=easing.continuousLoop, onComplete = function() ataqueChefe(chefe) end } )
    if(shot == false) then
...
function scene:create( event )
    ...
    uiDisp.AtualizaScore(tipo.totalScore);
