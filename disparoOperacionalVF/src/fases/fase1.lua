...
local som = require("src.audio.sons") -- funcoes de som
local imagens = require("src.carregarImagens") -- "fabrica de imagens"
local uiDisp = require("src.display.uiDisplay") -- funcoes de Display: vidas, placar, botoes
local heroi = require("src.heroi.Heroi") -- funcoes do heroi
local enemy = require("src.inimigo.Inimigo") -- funcoes dos inimigos
local item = require("src.itens.itens") -- funcoes dos itens
local transicao = require("src.fases.fases"); -- funcoes de transicao de tela
-------------------------------------------------------------------------------
...
local scrollSpeed = 2 -- Velocidade de movimento do fundo
...
local ultima = false -- marcador da ultima fase
...
local fundo = imagens.carregarFundo(1,backGroup)
fundo.x = display.contentCenterX 
fundo.y = display.contentCenterY
fundo.xScale = 1.0 
fundo.yScale = 1.0

local fundo2 = imagens.carregarFundo(1,backGroup)
fundo2.x = display.contentCenterX
fundo2.y = display.contentCenterY - display.actualContentHeight
fundo2.xScale = 1.0
fundo2.yScale = 1.0

parametrosFase = { fase = 1, imagem1 = fundo, imagem2 = fundo2, velocidade = scrollSpeed }
...
local function dispararEspecial(event)
    ...
    if (event.phase == "ended" or event.phase == "canceled") then
        ...
        timer.performWithDelay(500, desabilitaBotaoEspecial, 1)
    end   
end
    ...
    if(event.phase == "began")then
        ...
        elseif((obj2.myName == "laser" or obj2.myName == "nave" or obj2.myName == "especial") and obj1.myName == "chefe" )then
            ...
            if(lifeChefe == 0 )then
                ...
                display.remove(obj1);
                display.remove(escudoImg)
                local parametrosVitoria = { fase = 1, tipoNave = nave:getTipo(), totalVidas = nave:getVidas(), totalScore = score, totalMunicao = nave:getMunicao(), fim = ultima}
...
local function geraInimigo( event )
    ...
    local parametros = { fase = 1, grupo = mainGroup, vidas = nave:getVidas(), tipo = nil, vidaChefe = nil }

    if( nave:getVidas() ~= 0 )then
        local opt = math.random(2,4)
        parametros.tipo = opt
...
local function bigBoss( event )
    ...
    transition.to(chefe, {y = 60, time = 3000})
    local parametros = { fase = 1, grupo = mainGroup, vidas = nave:getVidas(), tipo = 1, vidaChefe = lifeChefe }
...
function scene:show( event )
	...
	elseif ( phase == "did" ) then
        ...
        chefeTimer = timer.performWithDelay(45000, chefeFinal, 1)
...
function scene:hide( event )
	...
	elseif ( phase == "did" ) then
        ...
        Runtime:removeEventListener("touch", botaoParametros.botaoLivre) 
        Runtime:removeEventListener("touch", botaoParametros.botaoEspecialLivre)
