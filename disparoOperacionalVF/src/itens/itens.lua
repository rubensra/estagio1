...
function itens.Especial(parametros)
    ...
    local paredeFogo =  display.newImageRect(parametros.grupo, itemSheet, parametros.tipo, 60, 80 )
    paredeFogo.x = math.random(50,260)
    paredeFogo.y = -100
    physics.addBody( paredeFogo, "dynamic", { radius = 10, isSensor=true } )
    paredeFogo.myName = "firewall"
    paredeFogo.xScale = 0.5
    paredeFogo.yScale = 0.5
    transition.to(paredeFogo, {y = display.contentHeight + 100, time = 5000, onComplete = function() display.remove(antiVirus) end } )
end
