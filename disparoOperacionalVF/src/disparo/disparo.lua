...
function disparo.HeroiEspecial(nave,grupo)

    local newLaser = tiroHeroi.carregarTiroEspecial(grupo)
    physics.addBody( newLaser, "dynamic", { radius = 5, isSensor=true } )
    newLaser.isBullet = true
    newLaser.myName = "especial" 
    newLaser.x = nave.x
    newLaser.y = nave.y
    newLaser:toBack()
    newLaser.xScale = 0.5
    newLaser.yScale = 0.5
    transition.to( newLaser, { y=-30, time=700, onComplete = function() display.remove( newLaser ) end } )
    som.somDisparo()
    return newLaser
end
...
function disparo.Chefe3(inimigo,tipo, grupo, vidas, vidaChefe,nave)
    if( inimigo.y ~= nil and vidas ~= 0 and vidaChefe ~= 0 )then
        local newLaser = tiroInimigo3.carregarTiro(grupo, tipo)
        physics.addBody( newLaser, "dynamic", { radius = 15, isSensor=true } )
        newLaser.isBullet = true
