...
local function gotoNext()
	...
	composer.gotoScene( "fase3", { time=800, effect="crossFade" } )
end
...
local background = display.newImageRect(backGroup, "imagens/fase2BG.png", 1200, 600)
background.x = display.contentCenterX
background.y = display.contentCenterY

 -- loading frasco de sangue
local  potSangue2 = display.newImageRect(mainGroup, "imagens/potsangue.png", 30, 30)
 potSangue2.x = display.contentCenterX+384
 potSangue2.y = display.contentHeight-452
 ...
 potSangue.x = display.contentCenterX+78
 potSangue.y = display.contentHeight-140
 ...
 placa.y = display.contentHeight-462
 placa.xScale = 1.3
 placa.yScale = 1.3
...
local tombstone = display.newImageRect(mainGroup, "imagens/tomb2.png", 40, 40)
    tombstone.x = display.contentCenterX-102  
    tombstone.y = display.contentHeight-145
    tombstone.xScale = 1.1
    tombstone.yScale = 1.1
...
local esk_sheet = graphics.newImageSheet("imagens/eskeleton.png", sheetOptionsEskeleto)
...
local eskeleton = display.newSprite(mainGroup, esk_sheet, eskeletoAndar)
 eskeleton.x = display.contentCenterX-35
 eskeleton.y = display.contentHeight-160
...
local function frenteTras()  
    if(vaiEvolta)then
        vaiEvolta = false
        transition.to(eskeleton, {x = display.contentCenterX+160, time = 4000, onComplete = function() inverte() frenteTras() end  })
        
    else
        vaiEvolta = true
        print("voltando")
        transition.to(eskeletonVoltar, {x = display.contentCenterX-160, time = 4000, onComplete = function() inverte() frenteTras() end   })
    end
end
--
...
local function enemyCollision(event)
	if(event.phase =="began") then
        if((event.target == obj2 and event.other == fantasma) or
            (event.target == obj2 and event.other == eskeleton) or
            (event.target == obj2 and event.other == eskeletonVoltar)) then
            died = true
            vidas = vidas - 1   
...
function bordaCollisionFim(event)
    if(event.phase == "began") then
        if (event.target == obj2 and event.other == bordaLateralFim) then
                motionx = 0
                motiony = 0
            if(qtdPotSangue == 3) then
                    composer.removeScene("fase2")
                    local parametros2 = { tempo = secondsLeft, life = vidas }
                    composer.gotoScene("fase3", {params = parametros2 })
            end
        end
    end
end
...
function scene:show( event )
    ...
    elseif ( phase == "did" ) then
        ...
        physics.addBody(potSangue, "static", {density = 5.0, radius= 40, bounce = 0})

        physics.addBody(potSangue2, "static", {density = 5.0, radius= 40, bounce = 0, isSensor = true})

      --  physics.addBody(potSangue2, "static", {density = 5.0, radius= 28, bounce = 0, isSensor = true})
        ...
        physics.addBody(tombstone, "static")
        
        physics.addBody(fantasma,"kinematic", {density = 14, radius = 40, bounce = 0.0})
        
        physics.addBody(eskeleton,"static", {density = 14, radius = 30, bounce = 0.0})
        
        physics.addBody(eskeletonVoltar, "static", {density = 14, radius = 40, bounce = 0.0})
